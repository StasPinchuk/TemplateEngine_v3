...
namespace TemplateEngine_v3.Services.UsersServices
{
    ...
    public class AllUsersManager
    {
        ...
        public ObservableCollection<UserModel> GetFilteredUsersList(UserModel currentUser, ObservableCollection<UserModel> excludedUsers)
        {
        ...
        {
            var allUsers = new UserReference(_connection).GetAllUsers().Where(user => IsFullNameValid(user.FullName)).ToList();
            _allUsers = new(UserMapper.FromTFlexUsersList(allUsers));
        }
